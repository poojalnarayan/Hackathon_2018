...
#         if len(fname) > 2:
#             continue
        if int(dictionary_.get( fname[-2], 0)) < int(fname[-1].strip('.')):
            dictionary_[fname[-2]] = fname[-1].strip('.')
    ...
    print("top_files_filtered = ", top_files_filtered)
...
def parse_email_data(path_to_files):
    ...
    email_files_list = listdir(path_to_files)
    for file in email_files_list:
        if(file.split("_")[-2] in filter_email_files_list):
            ...
            f.close()
    #print(tokens[1:5], " label =", labels[1:5])
    return tokens, labels
        
def import_emails(path_to_root):
    print("loading training data")
    train_sentences, train_labels = parse_email_data(path_to_root)
...
main("/work/pooja/dynet-base/dynet_tutorial/email_data/",test="/work/pooja/dynet-base/email_impersonation_detection/model_ruderdata", train="/work/pooja/dynet-base/email_impersonation_detection/model_ruderdata", num_epochs=2, batch_size=256, embedding_approach='pretrained')
